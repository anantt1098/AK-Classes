...
import { FaFileAlt, FaPlus, FaTrash, FaSearch, FaExternalLinkAlt } from "react-icons/fa";

import { uploadNote, getAllNotes, deleteNote } from "../../services/notes.service";
...
function Notes() {
    ...
    const fetchNotesList = async () => {
        try {
            ...
            toast.error("Failed to fetch uploaded notes.");
        } finally {
            setFetching(false);
        }
    };
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                        <FaFileAlt className="text-blue-600" /> Notes Management
                    </h1>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                        Upload & manage PDF notes class-wise and subject-wise. Total Uploaded: <span className="font-semibold text-slate-800 dark:text-slate-200">{notes.length}</span>
                    </p>
                </div>
                <button
                ...
                >
                    <FaPlus /> {showUploadForm ? "Hide Upload Form" : "Upload New Note"}
                </button>
            </div>
            ...
            {showUploadForm && (
                <FormCard>
                    <div className="mb-4 border-b border-slate-200 pb-3 dark:border-slate-800">
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Upload New Note</h2>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Provide note details and Google Drive / PDF link.</p>
                    </div>
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <Input
                            ...
                            placeholder="e.g., Chapter 4 Chemical Bonding Revision Notes"
                            required
                        />
            ...
            {fetching ? (
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <FaFileAlt className="mx-auto text-4xl text-slate-300 dark:text-slate-600" />
                    <h3 className="mt-3 text-lg font-bold text-slate-800 dark:text-slate-200">No Notes Uploaded Yet</h3>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Click "Upload New Note" above to add study materials for your students.</p>
                </div>
            ) : (
